'''
Time evolution of a Hasimoto soliton using non-linear binormal flow
gamma_t = k(1 + epsilon * k^2)B

status: needs to be checked and compared
'''

import numpy as np
import matplotlib.pyplot as plt
import matplotlib.animation as animation
import time


def soliton_gradient(f, h):
    """Non-periodic gradient function for solitons"""
    n = len(f)
    grad = np.zeros_like(f)

    # forward diff
    grad[0] = (f[1] - f[0]) / h

    # central diff
    for i in range(1, n-1):
        grad[i] = (f[i+1] - f[i-1]) / (2 * h)

    # backward diff
    grad[-1] = (f[-1] - f[-2]) / h

    return grad


def compute_curvature_binormal(gamma, h):
    """curvature and binormal using frenet-serret"""
    # first derivative - tangent
    d_gamma = np.zeros_like(gamma)
    for j in range(3):
        d_gamma[:, j] = soliton_gradient(gamma[:, j], h)

    # speed and unit tangent
    speed = np.linalg.norm(d_gamma, axis=1)
    speed = np.maximum(speed, 1e-10)  # Avoid division by zero
    tangent = d_gamma / speed[:, None]

    # second derivative - curvature direction
    curvature_vector = np.zeros_like(tangent)
    for j in range(3):
        curvature_vector[:, j] = soliton_gradient(tangent[:, j], h)

    curvature = np.linalg.norm(curvature_vector, axis=1)

    # normal
    normal = np.zeros_like(tangent)
    nonzero_curv = curvature > 1e-12
    normal[nonzero_curv] = curvature_vector[nonzero_curv] / curvature[nonzero_curv, None]

    # binormal
    binormal = np.cross(tangent, normal)

    return curvature, binormal, speed


def main():
    start_time = time.time()

    # soliton params
    num_points = 300
    s = np.linspace(-50, 50, num_points)
    t = 0

    nu = 1.0
    tau_0 = 0.5

    # parametrization
    eta = nu * (s - 2 * tau_0 * t)
    mu = 1 / (1 + tau_0**2)
    g = (2 * mu * (1 / np.cosh(eta))) / nu
    theta = tau_0 * s + (nu**2 - tau_0**2) * t

    x = s - (2 * mu / nu) * np.tanh(eta)
    y = g * np.cos(theta)
    z = g * np.sin(theta)

    gamma = np.stack((x, y, z), axis=1)

    # evolution params
    dt = 0.005  # time step
    n_steps = 1000
    h = (s[-1] - s[0]) / (num_points - 1)  # spatial step

    # non-linear parameter
    # want to try different values here
    epsilon = 0.1

    # animation params
    animation_steps = 50
    step_interval = max(1, n_steps // animation_steps)

    print(f"Computing evolution with epsilon = {epsilon}...")
    gamma_states = [gamma.copy()]
    gamma_current = gamma.copy()

    for step in range(n_steps):
        # compute curvature, binormal
        curvature, binormal, _ = compute_curvature_binormal(gamma_current, h)

        # non-linear binormal flow: gamma_t = k(1 + epsilon * k^2) * B
        # an expansion of gamma_t = kB
        nonlinear_factor = 1 + epsilon * curvature**2
        velocity = dt * curvature[:, None] * nonlinear_factor[:, None] * binormal

        # Stability check - limit velocity to prevent numerical blow-up
        velocity_magnitude = np.linalg.norm(velocity, axis=1)
        max_velocity = 0.02  # Adjust based on stability requirements
        scale_factor = np.minimum(1.0, max_velocity / np.maximum(velocity_magnitude, 1e-10))
        velocity *= scale_factor[:, None]

        # update curve
        gamma_current += velocity

        # Save states for animation
        if step % step_interval == 0:
            gamma_states.append(gamma_current.copy())

        # Progress reporting
        if step % 100 == 0:
            max_curvature = np.max(curvature)
            max_nonlinear = np.max(nonlinear_factor)
            print(f"Step {step}/{n_steps}, max curvature: {max_curvature:.6f}, max nonlinear factor: {max_nonlinear:.6f}")

            # Check for numerical issues
            if np.any(np.isnan(gamma_current)) or np.any(np.isinf(gamma_current)):
                print("WARNING: NaN or Inf detected in simulation!")
                break
    print(f"Total states computed: {len(gamma_states)}")

    # Set up the figure with multiple views
    fig = plt.figure(figsize=(12, 10))
    ax_xy = fig.add_subplot(221)                   # XY-plane
    ax_xz = fig.add_subplot(222)                   # XZ-plane
    ax_yz = fig.add_subplot(223)                   # YZ-plane
    ax_3d = fig.add_subplot(224, projection='3d')  # 3D plot

    # Compute plot limits
    all_x = np.concatenate([state[:, 0] for state in gamma_states])
    all_y = np.concatenate([state[:, 1] for state in gamma_states])
    all_z = np.concatenate([state[:, 2] for state in gamma_states])
    margin = 0.1
    xlim = (all_x.min() - margin, all_x.max() + margin)
    ylim = (all_y.min() - margin, all_y.max() + margin)
    zlim = (all_z.min() - margin, all_z.max() + margin)

    # Set limits and labels
    ax_xy.set_xlim(xlim)
    ax_xy.set_ylim(ylim)
    ax_xy.set_xlabel('X')
    ax_xy.set_ylabel('Y')
    ax_xy.set_title('XY-plane')
    ax_xy.grid(True, alpha=0.3)

    ax_xz.set_xlim(xlim)
    ax_xz.set_ylim(zlim)
    ax_xz.set_xlabel('X')
    ax_xz.set_ylabel('Z')
    ax_xz.set_title('XZ-plane')
    ax_xz.grid(True, alpha=0.3)

    ax_yz.set_xlim(ylim)
    ax_yz.set_ylim(zlim)
    ax_yz.set_xlabel('Y')
    ax_yz.set_ylabel('Z')
    ax_yz.set_title('YZ-plane')
    ax_yz.grid(True, alpha=0.3)

    ax_3d.set_xlim(xlim)
    ax_3d.set_ylim(ylim)
    ax_3d.set_zlim(zlim)
    ax_3d.set_xlabel('X')
    ax_3d.set_ylabel('Y')
    ax_3d.set_zlabel('Z')
    ax_3d.set_title(f'3D view (ε = {epsilon})')

    # Initialize line objects
    line_xy, = ax_xy.plot([], [], 'b-', lw=1.5)
    line_xz, = ax_xz.plot([], [], 'g-', lw=1.5)
    line_yz, = ax_yz.plot([], [], 'r-', lw=1.5)
    line_3d, = ax_3d.plot([], [], [], 'k-', lw=2)

    # Time label
    time_text = fig.text(0.02, 0.95, '', fontsize=12, weight='bold')
    # epsilon_text = fig.text(0.02, 0.90, f'Non-linear parameter ε = {epsilon}', fontsize=10)

    def animate(frame):
        if frame < len(gamma_states):
            gamma_frame = gamma_states[frame]

            x_vals = gamma_frame[:, 0]
            y_vals = gamma_frame[:, 1]
            z_vals = gamma_frame[:, 2]

            # Update 2D projections
            line_xy.set_data(x_vals, y_vals)
            line_xz.set_data(x_vals, z_vals)
            line_yz.set_data(y_vals, z_vals)

            # Update 3D plot
            line_3d.set_data_3d(x_vals, y_vals, z_vals)

            # Update time
            current_time = frame * step_interval * dt
            time_text.set_text(f'Time: {current_time:.4f}')

        return line_xy, line_xz, line_yz, line_3d, time_text

    # Create animation
    print("Creating animation...")
    anim = animation.FuncAnimation(
        fig, animate, frames=len(gamma_states),
        blit=False, repeat=True, interval=100
    )

    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"Elapsed time: {elapsed_time:.2f} seconds")

    plt.tight_layout()
    plt.show()

    print(f"Animation complete! Total evolution time: {n_steps * dt:.4f}")
    print(f"Non-linear correction strength: ε = {epsilon}")


if __name__ == "__main__":
    main()
